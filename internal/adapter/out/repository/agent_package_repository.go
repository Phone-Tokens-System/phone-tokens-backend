...
func (r *AgentPackageRepository) AddAgentPackageTx(ctx context.Context, tx *gorm.DB, req *model.AgentPackages) error {
...
func (r *AgentPackageRepository) GetAgentPackageByIdTx(ctx context.Context, tx *gorm.DB, id int) (*model.AgentPackages, error) {
	var pkg model.AgentPackages
	err := tx.WithContext(ctx).Set("gorm:query_option", "FOR UPDATE").Find(&pkg, "id = ?", id).Error
	if err != nil {
		return nil, err
	}
	return &pkg, nil
...
func (r *AgentPackageRepository) UpdateAgentPackageTx(ctx context.Context, tx *gorm.DB, id int, pkg *model.AgentPackages) (*model.AgentPackages, error) {
	...
	var updated model.AgentPackages
	if err := tx.WithContext(ctx).First(&updated, id).Error; err != nil {
		return nil, err
	}
